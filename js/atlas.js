...
(function () {
  ...
  // Every element this file touches, checked in ONE place and named when it is
  // absent.
  //
  // This guard used to cover four of the seven, and `main` was not among them
  // even though fitIndex() calls getComputedStyle(main) unconditionally. The
  // failure that produced was maximally unhelpful: the world map drew, the
  // index drew, routing switched views — and then a TypeError three hundred
  // lines away left the region with its name, its count and its sheet all
  // missing, looking for all the world like a CSS or a data problem. It is
  // neither. It is one missing element, and the page should say so.
  //
  // Returning early rather than limping on: half an atlas is not better than
  // none, and a console line naming the selector is worth more than a blank
  // panel.
  var NEEDED = [
    ['.atlas-map',    map],
    ['.region-index', index],
  ...
  for (var n = 0; n < NEEDED.length; n++)
    if (!NEEDED[n][1]) missing.push(NEEDED[n][0]);
  if (missing.length) {
    if (window.console && console.error)
      console.error('atlas.js: this page is missing ' + missing.join(', ') +
                    ' — nothing will be drawn. Every one of these must be in ' +
                    'the document before js/atlas.js runs.');
    return;
  }
  ...
  // The three inside the region view, checked the same way. showRegion writes
  // to all three on its first three lines, so a missing one takes the region
  // down at the moment it opens rather than at load — which is why this looked
  // like the sheet was failing to build.
  var INNER = [
    ['.region-title', title],
    ['.region-note',  note],
    ['.region-stage', stage]
  ...
  for (var m = 0; m < INNER.length; m++) {
    if (INNER[m][1]) continue;
    if (window.console && console.error)
      console.error('atlas.js: #atlas-region is missing ' + INNER[m][0] +
                    ' — the region view cannot be drawn.');
    return;
  }
  ...
  function fitIndex() {
    ...
    var pad = main ? parseFloat(getComputedStyle(main).paddingBottom) || 0 : 0;
