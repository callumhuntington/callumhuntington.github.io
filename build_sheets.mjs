...
import {REGIONS, SPLIT_SUBUNITS, SPLIT_STATES, SUBUNIT_ALIAS} from './regions.mjs';
...
// ── the states ──
// Simplified on its own quantile rather than pooled with the coastlines above,
// which is the whole reason it is a separate file: see the note in
// prepare_subunits.mjs. Optional — a checkout that has not re-run the prepare
// step still builds, and simply draws no state borders.
const readStates = () => JSON.parse(readFileSync('states-topo.json'));
let stateTopo = null, stateUnits = [];
if (SPLIT_STATES.size && existsSync('states-topo.json')) {
  const sw = quantile(presimplify(readStates()), RETAIN);
  stateTopo = simplify(presimplify(readStates()), sw);
  stateUnits = stateTopo.objects.units.geometries;
} else if (SPLIT_STATES.size) {
  console.warn('states-topo.json missing — run prepare_subunits.mjs to draw ' +
               'state borders for: ' + [...SPLIT_STATES].join(', '));
}

// The internal borders of every SPLIT_STATES country in a region, as one mesh:
// exactly the arcs two different divisions have in common. Null when the
// region contains none of them, which is every region but one.
function stateMesh(admins) {
  if (!stateTopo) return null;
  const mine = stateUnits.filter(g => admins.has(g.properties.admin) &&
                                      SPLIT_STATES.has(g.properties.admin));
  if (mine.length < 2) return null;
  return tc.mesh(stateTopo, {type: 'GeometryCollection', geometries: mine},
    (a, b) => a !== b);
}
...
for (const key of Object.keys(REGIONS)) {
  ...
  let borders = ls.result();

  // State borders, where the region has a country that draws them. Appended to
  // the same path rather than given a layer of their own: atlas.js already
  // strokes .sheet-borders in white and clips it to the silhouette, and a
  // state line wants exactly that treatment.
  const sm = stateMesh(admins);
  if (sm) {
    const ss = lineSink();
    geoPath(proj, ss)(sm);
    borders += ss.result();
