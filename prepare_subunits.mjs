#!/usr/bin/env node
/**
 * prepare_subunits.mjs — builds subunits-topo.json, states-topo.json and
 *                        cities-10m.json
 *
 ...
 *
 * That layer stops there, though — it has no US states in it, and no Italian
 * regions either. Anything at that level comes from the admin-1 layer instead,
 * for the countries named in SPLIT_STATES, and goes into its own topology; see
 * the note above that section for why it is kept separate.
 *
 * Neither layer is on npm, so this script fetches them once, throws away the
 * hundreds of countries the atlas has no use for, and writes small TopoJSON
 * files that sit beside the build scripts and are never served. Re-run it when
...
import {REGIONS, SUBUNIT_ALIAS, SPLIT_STATES} from './regions.mjs';
...
// ── the states ───────────────────────────────────────────────────────────────
// A SEPARATE topology, deliberately, rather than substituting these features
// for the country's own in the one above.
//
// build_sheets.mjs simplifies the subunit topology as a whole and picks its
// threshold as a QUANTILE over every arc in it. Dropping fifty state borders
// into that pool moves the quantile, which changes how hard every OTHER
// region's coastline is simplified, which moves every card the placement
// solver has to fit around — including the ones placed by hand in photos.js.
// Adding borders to one sheet is not worth silently re-cutting the other
// thirteen.
//
// The cost of keeping them apart is that state arcs are not shared with the
// coastline arcs, so a state boundary reaching the sea can overshoot or fall
// a hair short of it. Neither shows: atlas.js clips the border path to the
// silhouette, which trims any overshoot, and a shortfall is well under a
// pixel at sheet scale.
const STATE_URL = 'https://raw.githubusercontent.com/nvkelso/natural-earth-vector/' +
                  'master/geojson/ne_50m_admin_1_states_provinces.geojson';
const STATE_CACHE = 'ne_50m_admin_1_states_provinces.geojson';

if (!SPLIT_STATES.size) {
  console.log('\nno countries in SPLIT_STATES — skipping states-topo.json');
} else {
  let adm1;
  if (existsSync(STATE_CACHE)) {
    adm1 = JSON.parse(readFileSync(STATE_CACHE));
    console.log('\nusing cached', STATE_CACHE);
  } else {
    console.log('\nfetching', STATE_URL);
    adm1 = await (await fetch(STATE_URL)).json();
    writeFileSync(STATE_CACHE, JSON.stringify(adm1));
  }

  // This layer spells the property lowercase where the admin-0 files shout it.
  const adminOf = f => f.properties.admin ?? f.properties.ADMIN;
  const stateFeats = adm1.features.filter(f => SPLIT_STATES.has(adminOf(f)));

  const gotStates = new Set(stateFeats.map(adminOf));
  const noStates = [...SPLIT_STATES].filter(c => !gotStates.has(c));
  if (noStates.length)
    throw new Error('not in the admin-1 layer: ' + noStates.join(', '));

  const cleanStates = stateFeats.map(f => ({
    type: 'Feature',
    properties: {admin: adminOf(f), subunit: f.properties.name},
    geometry: f.geometry,
  }));

  const stateTopo = topology(
    {units: {type: 'FeatureCollection', features: cleanStates}}, 1e5);
  writeFileSync('states-topo.json', JSON.stringify(stateTopo));

  const byCountry = {};
  for (const f of cleanStates) (byCountry[f.properties.admin] ||= []).push(1);
  for (const [c, n] of Object.entries(byCountry).sort())
    console.log('  ' + c.padEnd(26) + n.length + ' divisions');
  console.log(`wrote states-topo.json — ` +
              `${(JSON.stringify(stateTopo).length / 1024).toFixed(0)}kB`);
}
